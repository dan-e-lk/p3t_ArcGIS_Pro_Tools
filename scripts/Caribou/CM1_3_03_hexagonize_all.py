...
def hexagonizer(hex_base_gdb,out_gdb,join_info,mu_list):
	...
	for mu in mu_list:
		...
		for layer_type, info in join_info.items():
			fname_prefix = info[0]
			join_full_path = info[1]
			join_sql = info[2]
			join_exclude_field_list = info[3]
...
if __name__ == '__main__':
	...
	join_info = {
	# layer alias | fieldname prefix | path to join layer | sql select | exclude these fields
	'EventLayer': 	[None, neofin_path, None, None],
	'Harvest':		['HRV', hrv_path, None, ['HRVNO']],
	}
